...
class LogController extends Controller
{
    ...
    public function update(Request $request, ExerciseLog $log)
    {
        ...
        $duplicate = ExerciseLog::query()
            ...
            ->where('set_number', $data['set_number'])
            ->whereDate('logged_at', $log->logged_at)
            ->exists();

        if ($duplicate) {
            return back()->withErrors([
                'set_number' => "Ya existe otra serie {$data['set_number']} ese día para este ejercicio. Elegí otro número o borrá la otra primero.",
            ]);
        }
